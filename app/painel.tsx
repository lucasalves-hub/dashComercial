...
export default function Painel({ dados, email, sair }: { dados: any; email: string; sair: () => Promise<void> }) {
  ...
  return <main>
    ...
    <section className="cards"><Card titulo="INVESTIMENTO COMERCIAL TOTAL" valor={brl.format(v.realizado)} descricao="Carteira: ganhas, negociação e estrutura" destaque /><Card titulo="CUSTO DE AQUISIÇÃO (GANHAS)" valor={brl.format(v.aquisicao)} descricao={v.realizado ? `${percentual(v.aquisicao / v.realizado * 100)} do investimento total` : 'Sem base no período'} /><Card titulo="ORÇAMENTO CONSUMIDO" valor={percentual(consumo)} descricao={periodo === '2026' ? `Esperado até ${meses[v.ultimo] || 'agora'}/26: ${percentual(v.q / 12 * 100)} · ${brl.format(esperado)}` : brl.format(v.verba)} /><Card titulo="SALDO DISPONÍVEL" valor={periodo === 'ambos' ? '—' : brl.format(v.verba - v.realizado)} descricao="Capacidade remanescente do orçamento" /></section>
    <section className="grid"><article><h2>Ritmo de investimento</h2><p>Comparativo somente das competências já realizadas em 2026. Meses futuros não são interpretados como economia.</p><p style={{ fontSize: 11, marginTop: -8 }}><span style={{ color: '#a99dad' }}>■ 2025</span> &nbsp; <span style={{ color: '#8127a6' }}>■ 2026</span></p><div className="chart">{competencias.map((x: any) => <div key={x.mes}><i className="old" style={{ height: `${x.a25 / maximo * 170}px` }} /><i className="new" style={{ height: `${x.a26 / maximo * 170}px` }} /><small>{x.mes}</small></div>)}</div></article><article><h2>Leitura de controladoria</h2><dl><dt>Mesmo período de 2025</dt><dd>{brl.format(v.mp25)}</dd><dt>Variação de investimento</dt><dd className={variacao > 0 ? 'risk' : 'ok'}>{percentual(variacao)}</dd><dt title="Orçamento anual distribuído linearmente pelos meses já decorridos.">Consumo esperado até agora</dt><dd>{periodo === '2026' ? brl.format(esperado) : '—'}</dd><dt>{desvio >= 0 ? 'Acima do ritmo orçado' : 'Abaixo do ritmo orçado'}</dt><dd className={desvio > 0 ? 'risk' : 'ok'}>{periodo === '2026' ? `${desvio > 0 ? '+' : '−'}${brl.format(Math.abs(desvio))}` : '—'}</dd><dt>No ritmo atual, 2026</dt><dd>{brl.format(v.r26 / v.q * 12)}</dd></dl></article></section>
